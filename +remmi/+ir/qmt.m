...
% MDD commented out several return variables to save space. We need a
% better way to choose the output parameters of interest
...
fop = optimset(fop,'display','off','TolX',1e-3,'TolFun',1e-4);
...
for n=1:numel(mask)
  if mask(n)
    ...
    % fit the data
    b = lsqnonlin(@(x) remmi.ir.sir(x,ti',td)-sig,...
      b0,lb,ub,fop);
