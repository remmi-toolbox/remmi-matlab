function dset = loadImg(dset,options)
% dset = remmi.loadImg(dset,options) loads saved images from recognized 
...
%   options.id = recon ID to load, default is '1'
%
% output:
...
for n=1:length(exps)
    [dset.img{n},dset.labels{n},dset.pars{n}] = study.loadImg(num2str(exps{n}),options);
